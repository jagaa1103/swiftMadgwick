...
class Madgwick {
    ...
    // IMU ALGORITHM
    func madgwickAHRS_IMU(gx: Double, gy: Double, gz: Double, ax: Double, ay: Double, az: Double) -> (Double, Double, Double, Double) {
        ...
        if !(_ax == 0.0 && _ay == 0.0 && _az == 0.0) {
            ...
            recipNorm = pow(_ax * _ax + _ay * _ay + _az * _az, -0.5)
            _ax *= recipNorm
            _ay *= recipNorm
            _az *= recipNorm
